...
struct perms_config {
    ...
    perms_config(char *filename) {
        ...
        gen = mt19937(std::chrono::high_resolution_clock::now().time_since_epoch().count());
        distrib = uniform_int_distribution<>(0, INT_MAX);
        ...
        if (parent_pool > population_size) {
            Log::die("Error: Parent pool cannot be larger than population size.");
        }
